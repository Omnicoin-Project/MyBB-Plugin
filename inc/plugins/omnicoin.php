...
function omnicoin_activate() {
	...
	$AddressSearchResultsTemplate = array(
		"tid"			=> NULL,
	...
	$db->insert_query("templates", $AddressSearchTemplate);
	$db->insert_query("templates", $AddressSearchResultsTemplate);
...
function omnicoin_member_profile_start() {
	...
	if ($query->num_rows == 1) {
		...
		$omcbalance = omnicoin_get_user_balance(intval($mybb->input['uid']));
...
function omnicoin_user_update($userhandler) {
	...
	if ($mybb->input['action'] == "do_profile") {
		...
		if (isset($mybb->input['omc_address']) && isset($mybb->input['omc_signature']) && !empty($mybb->input['omc_address'])) {
			...
			$address = preg_replace("/[^A-Za-z0-9]/", "", $db->escape_string($mybb->input['omc_address']));
-			$signature = preg_replace("/[^A-Za-z0-9=+-\/]/", "", $db->escape_string($mybb->input['omc_signature']));
	...
	if (isset($mybb->input['action'])) {
		if ($mybb->input['action'] == "omchistory") {
			...
			if (isset($mybb->input['search'])) {
				$search = preg_replace("/[^A-Za-z0-9]/", "", $db->escape_string($mybb->input['search']));
				...
				if ($query->num_rows > 0) {
					...
					while($row = $db->fetch_array($query)){
						...
						$addresses = $addresses . "<tr class='trow1'><td><a target='_blank' href='https://omnicha.in?address=" . $row['address'] . "'>" . $row['address'] . "</a></td><td><a href='member.php?action=profile&uid=" . $row['uid'] . "'>" . $username . "</td><td>" . $row['date'] . "</td></tr>";
					}
				} else {
					$addresses = "<tr class='trow1'><td colspan=3>No results found</td></tr>";	
				}
